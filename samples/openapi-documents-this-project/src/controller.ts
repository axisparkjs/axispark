import { Body, Controller, Cookie, Delete, Get, Header, NotFoundError, Param, Post, Put, Query } from '@axisparkjs/http';
...
export class TodoController {
    ...
    @Get()
    @OpenApiResponse({ statusCode: 200, type: Todo, isArray: true, description: 'Returns a list of todos' })
    getTodos() {
    ...
    getTodoByIndex(@Param('index') @Pipe(ParseInt(10)) index: number, @Query('includeCompleted') _includeCompleted: boolean) {
    ...
    @OpenApiResponse({ statusCode: 404, description: 'Todo not found' })
    @OpenApiResponse({ statusCode: 400, description: 'Bad request' })
    updateTodo(@Param('index') @Pipe(ParseInt(10)) index: number, @Body() @Pipe(NotEmpty()) updatedTodo: Todo) {
    ...
    @OpenApiResponse({ statusCode: 404, description: 'Todo not found' })
    removeTodo(@Param('index') @Pipe(ParseInt(10)) index: number, @Header('Authorization') _authHeader: string) {
