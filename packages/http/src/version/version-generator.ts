...
export class VersionGenerator implements Generator<VersionDefinition | undefined> {
    ...
    generate(context: HttpContext): VersionDefinition | undefined {
        ...
        const controllerVersions = controllerMetadata?.version ? typeof controllerMetadata.version === 'string' ? [controllerMetadata.version] : controllerMetadata.version : [];
        ...
        if (this.versionOptions.type === VersionType.Uri && this.versionOptions.defaultVersion)
            acceptedVersions.push(this.versionOptions.defaultVersion);
