...
describe('HttpTimeoutProcessor', () => {
    const createTimeout = (time: number): TimeoutDefinition =>
        ({ time } as TimeoutDefinition);
    ...
    it('should throw RequestTimeoutError with the default message', async () => {
        ...
        await expect(
            processor.resolve(createTimeout(5000))
        ).rejects.toThrow(
            new RequestTimeoutError('Request timed out after 5000ms')
        );
    });
    ...
    it('should throw RequestTimeoutError with the provided string message', async () => {
        ...
        await expect(
            processor.resolve(createTimeout(3000))
        ).rejects.toThrow(
            new RequestTimeoutError('Custom timeout message')
        );
    });

    it('should use the message function with the timeout', async () => {
        const message = jest.fn(
            (time: number) => `Request exceeded ${time}ms`
        );
        ...
        await expect(
            processor.resolve(createTimeout(2500))
        ).rejects.toThrow(
            new RequestTimeoutError('Request exceeded 2500ms')
        );
    ...
    it('should use the default message when message is an empty string', async () => {
        ...
        await expect(
            processor.resolve(createTimeout(1000))
        ).rejects.toThrow(
            new RequestTimeoutError('Request timed out after 1000ms')
        );
    });
    ...
    it('should use the default message when timeout options are not provided', async () => {
        ...
        await expect(
            processor.resolve(createTimeout(7500))
        ).rejects.toThrow(
            new RequestTimeoutError('Request timed out after 7500ms')
        );
    });
    ...
    it('should use the default message when message is undefined', async () => {
        ...
        await expect(
            processor.resolve(createTimeout(2000))
        ).rejects.toThrow(
            new RequestTimeoutError('Request timed out after 2000ms')
        );
    });
});