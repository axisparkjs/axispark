...
export class FileSystemScanner implements Scanner {
    ...
    /**
     * Scans the file system for JavaScript and TypeScript files starting from the specified root directory. The method uses the `fast-glob` library to find files matching the patterns **.{js,ts} while ignoring certain directories and file types, such as `node_modules`, `dist`, and test files. Once the matching files are found, they are imported using the provided importer function.
     */
